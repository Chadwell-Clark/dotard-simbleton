...
const newYorkBusinesses = businesses.filter(
...
const purchasingAgents = businesses.map((business) => {
  const container = {};
  (container.fullName = `${business.purchasingAgent.nameFirst} ${business.purchasingAgent.nameLast}`),
    (container.company = business.companyName),
    (container.phoneNumber = business.phoneWork);
  return container;
});
